...
import { User, MapPin, Building2, ChevronDown, ChevronUp, Mail, Phone, Linkedin, ExternalLink, Target, Zap, X, Copy, RefreshCw, Send, ClipboardCheck, Edit3, Bookmark, Star, Map } from 'lucide-react';
    ...
    return (
      <div className="flex items-center gap-0.5">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star 
            key={star} 
            className={`w-3.5 h-3.5 ${star <= Math.round(rating) ? 'fill-yellow-400 text-yellow-400' : 'text-slate-600'}`} 
          />
        ))}
        <span className="text-slate-300 text-sm ml-2 font-medium">{rating}</span>
        <span className="text-slate-500 text-xs ml-1">({lead.reviewCount})</span>
      </div>
    );
  };
          ...
          <div className="flex items-start gap-4 flex-1">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-2 mb-1">
                 ...
                 <div className="flex items-center gap-1 text-slate-400 text-xs truncate max-w-[200px]">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700/50 h-full">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-300 border-t border-slate-800 pt-2 mt-2">
